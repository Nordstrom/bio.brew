...
Simple package manager for cool tools.
Fork of "drio's biobrew":https://github.com/drio/bio.brew with some slight architecture changes.

Allows you to write _recipes_ that describe how to install your favorite tool. Recipes are written in bash.
Once a recipe is written, you can install it simply with:
...
See "bitter.brew's example recipe":https://github.com/vlandham/bio.brew/blob/master/recipes/example.sh to get a feel for how a recipe should look. 

In practice, a new recipe will usually start as a copy of an existing recipe tweaked to work.

h2. Install

There are a few ways to install and configure bitter.brew. My (current) preferred method involves *checking out the source from github using git* and then using the *bb_load_env* setup as detailed below. The requirement for this method is of course that you need git installed on your system.

h3. Install with git

git clone git://github.com/Nordstrom/bitter.brew.git
...
@BB_PATH@ defines the *location of the bitter.brew package*, while @BB_INSTALL@ tells bitter.brew *where to install tools*.

This will add the bb executable to your path as well as binaries created using bitter.brew. Just adding these variables will *not* add the recipe specific ENV variables to your path (see below).

Here is an example setup for basic bitter.brew Brew use. This example assumes you have the bio.brew tool downloaded to the directory: @/home/username/bio.brew@ and want bio.brew to install tools at @/home/username/tools@

<pre>
export BB_INSTALL=/home/username/tools
export BB_PATH=/home/username/tools/bitter.brew
...
Here we use bitter.brew to install samtools. The -j8 is passed to make when compiling
...
Recipe creation is an iterative process. Usually you won't get it completely right the first time. In the future, we hope to make @bitter.brew@ more accommodating to this process. For now, there are a few things you can do.

h4. Run bb clean <tool>

If @bitter.brew@ dies during one of its steps, it could leave behind lock files that might prevent running the installation process again.
...
This will remove the meta files @bitter.brew@ creates, allowing you to re-run the recipe (after you've fixed the problem).
...
@bitter.brew@ comes with a handy @log@ helper, which you can use to echo out relevant information during the installation of your recipe
...
h2. How I Use bitter.brew

As an example setup, here is how I've installed and setup bitter.brew on OSX 10.5.8. Setup should be very similar for other versions of OS X as well as Linux distributions.

First, I created a @tools@ directory in my home directory and cloned the bitter.brew repo there:
...
export BB_PATH=~/tools/bitter.brew
PATH="$BB_INSTALL/bin:$BB_PATH/bin:$PATH"
export PATH

[[ -s "$BB_INSTALL/bitter.brew/bb_load_env" ]] && source "$BB_INSTALL/bitter.brew/bb_load_env"
</pre>

This indicates that all my bitter.brew recipes will be installed in the @bin@ directory under @~/tools@

As I manage java, subversion, and other packages externally of bitter.brew, I first fake these inside of bitter.brew to satisfy dependencies
...
Now I'm ready to use bitter.brew to install and manage my bioinformatic packages!
...
BB currently works in Linux enviroments (it should work just fine in other flavours of Unix). It only needs a typical Linux enviroment with gcc, curl (and potentially git). Besides classic and inseparable unix tools, bitter.brew's focus is on bioinformatic tools and packages. 

bitter.brew has a similar feel to it as the "homebrew":https://github.com/mxcl/homebrew package manager for Mac OSX. But it isn't as good.
The difference being that it is much more lean in terms of functionality and packages provided, it is written entirely in shell instead of a mixture of shell and ruby, it works on more than just Mac OSX, and its focus is only on core bioinformatic tools.

Give it a try and make it better by improving the framework and adding more recipes.

h2. Differences from Original bitter.brew
